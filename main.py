...
window.title("DeadLift_Counter") 
ck.set_appearance_mode("dark")

#differents widgets placed in the GUI 
...
def reset_counter(): 
    global counter
    counter = 0 
#RESET button and placement on the app
...
#tracking different movements in the guyi
mp_drawing = mp.solutions.drawing_utils
mp_pose = mp.solutions.pose
pose = mp_pose.Pose(min_tracking_confidence=0.5, min_detection_confidence=0.5) 

#this line of code opens up the deadlift.pkl file
with open('deadlift.pkl', 'rb') as f: 
    model = pickle.load(f) 

#video capturing  variables
...
#detection function for the action
def detect(): 
...
# creating different landmarks on the window screen
    ret, frame = cap.read()
    image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) 
    results = pose.process(image)
    mp_drawing.draw_landmarks(image, results.pose_landmarks, mp_pose.POSE_CONNECTIONS, 
        mp_drawing.DrawingSpec(color=(106,13,173), thickness=2, circle_radius = 2), 
        mp_drawing.DrawingSpec(color=(255,102,0), thickness=2, circle_radius = 2)) 
...
#function recall in the main loop
detect() 
window.mainloop()